...
import MozartPassportTabs from "@/components/composer/MozartPassportTabs";
...
export default function MozartComposerPassportPage() {
  const composer = getComposerBySlug("wolfgang-amadeus-mozart")!;
  const facts = [
    ["Родился", "1756"],
    ["Умер", "1791"],
    ["Страна", "Австрия"],
    ...
    ["Паспорта", String(mozartWorks.length).padStart(2, "0")],
  ];
    ...
    <main className="min-h-screen bg-[#f6f2ea] text-[#171714] lg:pl-[216px] lg:pr-[310px]">
        ...
        <div className="mx-auto max-w-[1100px]">
          <div className="mb-5 flex items-center justify-between text-xs text-black/45">
            <div className="hidden items-center gap-3 md:flex">
              ...
              <span>›</span>
              <strong className="font-medium text-black/70">Вольфганг Амадей Моцарт</strong>
            </div>
          ...
          <section className="relative min-h-[350px] overflow-hidden rounded-[1.7rem] border border-black/10 bg-[#e9dfcf] shadow-[0_20px_60px_rgba(60,45,20,.08)]">
            ...
            <div
              className="absolute bottom-0 right-0 h-full w-[56%] bg-cover bg-[50%_17%] sepia-[.1] contrast-110"
              style={{ backgroundImage: `url('${portrait}')` }}
            />
            <div className="absolute inset-y-0 left-[44%] w-40 bg-gradient-to-r from-[#e9dfcf] via-[#e9dfcf]/88 to-transparent" />
            <div className="absolute inset-0 bg-gradient-to-t from-[#8d6a31]/10 via-transparent to-white/10" />
            ...
            <div className="relative z-10 max-w-[58%] px-7 py-10 sm:px-10 sm:py-12">
              <p className="mb-5 text-xs uppercase tracking-[.24em] text-[#9b7130]">
                Composer Passport · PA-COMP-MOZ-0001
              </p>
              <h1 className="font-serif text-4xl leading-[.94] tracking-[-.035em] sm:text-6xl">
                Вольфганг Амадей Моцарт
              </h1>
              <p className="mt-5 font-serif text-xl text-black/55">1756–1791</p>
              ...
              <div className="mt-7 flex flex-wrap gap-2 text-[10px] uppercase tracking-[.12em] text-[#795925]">
                ...
                <span className="rounded-full border border-[#9b7130]/25 bg-white/45 px-4 py-2">Оперный театр</span>
              </div>
            </div>
          ...
          <MozartPassportTabs works={mozartWorks} />
        </div>
      </section>
      ...
      <aside className="fixed inset-y-0 right-0 z-30 hidden w-[310px] overflow-y-auto border-l border-black/10 bg-[#fbf8f2] p-5 lg:block">
        <section className="rounded-2xl border border-black/10 bg-white/65 p-5 shadow-sm">
          ...
          <p className="mt-1 text-sm text-black/50">№ PA-COMP-MOZ-0001</p>

          <div className="mt-5 overflow-hidden rounded-xl border border-black/10">
            <div className="relative h-48 bg-[#ded5c8]">
              <div className="absolute inset-0 bg-cover bg-[50%_17%] sepia-[.1]" style={{ backgroundImage: `url('${portrait}')` }} />
              <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
              <p className="absolute bottom-4 left-4 right-4 text-xs leading-5 text-white/85">
                Нажмите на большой портрет: лицо Моцарта будет двигаться и взрослеть во время скролла.
              </p>
            </div>
          </div>
        ...
        <section className="mt-5 rounded-2xl border border-black/10 bg-white/65 p-5">
          ...
          <div className="mt-5 space-y-3 text-sm text-black/62">
            ...
            <div className="flex items-center justify-between border-b border-black/8 pb-3"><span>Произведения</span><span>{String(mozartWorks.length).padStart(2, "0")}</span></div>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-50 grid grid-cols-5 border-t border-black/10 bg-[#fffdf8]/95 px-2 py-2 backdrop-blur lg:hidden">
        {[["⌑", "Атлас"], ["♙", "Композиторы"], ["♫", "Произведения"], ["▣", "Библиотека"], ["⌕", "Поиск"]].map(([icon, label]) => (
          <a key={label} href="#" className={`grid place-items-center gap-1 text-[10px] ${label === "Композиторы" ? "text-[#9b7130]" : "text-black/50"}`}>
