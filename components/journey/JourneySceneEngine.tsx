...
type ParticleMode = "leaves" | "dust" | "none";
type OverlayVariant = "forest" | "interior" | "neutral";
...
const dust = Array.from({ length: 38 }, (_, index) => ({
  ...
  top: `${4 + ((index * 29) % 88)}%`,
  size: 1 + (index % 4) * 0.7,
  delay: `${-(index % 12) * 1.4}s`,
  duration: `${10 + (index % 8) * 2.2}s`,
  drift: -18 + ((index * 17) % 36),
  opacity: 0.18 + (index % 5) * 0.08,
}));

const overlayStyles: Record<OverlayVariant, string> = {
  forest:
    "linear-gradient(90deg,rgba(4,4,3,.9)_0%,rgba(5,5,4,.55)_26%,transparent_54%,rgba(5,5,4,.18)_77%,rgba(3,3,2,.68)_100%)",
  interior:
    "linear-gradient(90deg,rgba(8,6,4,.82)_0%,rgba(10,7,4,.4)_34%,transparent_58%,rgba(6,4,3,.5)_100%)",
  neutral:
    "linear-gradient(90deg,rgba(4,4,3,.78)_0%,rgba(5,5,4,.35)_34%,transparent_64%,rgba(3,3,2,.55)_100%)",
};
...
export default function JourneySceneEngine({
  ...
  return (
    ...
    >
        ...
        style={{
          ...
          background:
            overlayVariant === "interior"
              ? "linear-gradient(112deg,transparent 8%,rgba(255,224,169,.17) 30%,transparent 49%),radial-gradient(ellipse at 66% 18%,rgba(255,231,180,.14),transparent 34%)"
      ...
      <div className="absolute inset-0" style={{ background: overlayStyles[overlayVariant] }} />
            ...
            return (
                ...
                style={
                  {
                    ...
                    filter: `blur(${leaf.blur}px) drop-shadow(0 ${Math.max(1, leaf.size * 0.08)}px ${Math.max(1, leaf.size * 0.12)}px rgba(0,0,0,.28))`,
              ...
              >
                ...
                >
                  ...
                  <path d="M2.8 17.2C5.4 12.7 7.2 8.4 12.1 5.7L15.1 6.3L17.8 3.4L20.5 5.1L23.1 2.8L25.7 5.2L29.2 4.9L31 8L34.7 8.8L34.1 11.7L39.5 15.3L37.1 18.2L40.1 21.2L35.4 22.6L34 26.1L29.8 25.8L26.7 29.2L22.8 27.8L19.4 30.8L16.9 27.3L12.4 27.6L10.9 23.7L6.8 22.6L7.1 19.5Z" fill={`url(#${gradientId})`} />
                  <path d="M4.6 17.9C13.1 17.1 22.1 16.5 37.2 16.1" fill="none" stroke={palette.vein} strokeWidth="1.15" strokeLinecap="round" opacity=".82" />
                  <path d="M13.4 17.1L10.1 11.6M17.3 16.8L15.1 8.2M21.3 16.6L20.4 6.1M25.1 16.4L27.6 7.2M29.1 16.3L33 10.7M12.8 18L10.4 23.1M17 17.6L15.6 25.2M21.2 17.2L21.2 27.2M25.4 17L27.7 25.8M29.2 16.8L33.1 22.4" fill="none" stroke={palette.vein} strokeWidth=".62" strokeLinecap="round" opacity=".58" />
                  <path d="M2.8 17.7C1.5 18.3.7 19.1 0 20" fill="none" stroke={palette.vein} strokeWidth="1" strokeLinecap="round" />
                </svg>
              </span>
            );
          })}
      ...
      {particles === "dust" && (
        <div className="pointer-events-none absolute inset-0 overflow-hidden">
          {dust.map((particle, index) => (
            <span
              key={index}
              className="absolute rounded-full bg-[#f2d8a0] shadow-[0_0_12px_rgba(255,223,161,.55)] animate-[journeyDustFloat_ease-in-out_infinite]"
              style={
                {
                  left: particle.left,
                  top: particle.top,
      ...
      <style jsx>{`
        @keyframes journeyLeafFall {
          0% { transform: translate3d(0,-14vh,0) rotateZ(var(--leaf-start-rotation)) scale(.84); opacity: 0; }
          8% { opacity: var(--leaf-opacity); }
          30% { transform: translate3d(calc(var(--leaf-drift) * .18),29vh,0) rotateZ(calc(var(--leaf-start-rotation) + 130deg)) scale(.95); }
          58% { transform: translate3d(calc(var(--leaf-drift) * .68),67vh,0) rotateZ(calc(var(--leaf-start-rotation) + 330deg)) scale(1.04); }
          82% { opacity: var(--leaf-opacity); }
          100% { transform: translate3d(var(--leaf-drift),122vh,0) rotateZ(calc(var(--leaf-start-rotation) + var(--leaf-spin))) scale(.9); opacity: 0; }
        }
        @keyframes journeyLeafFlutter {
          0%,100% { transform: rotateX(18deg) rotateY(-34deg) rotateZ(-7deg) skewX(-2deg); }
          22% { transform: rotateX(-31deg) rotateY(18deg) rotateZ(10deg) skewX(3deg); }
          49% { transform: rotateX(28deg) rotateY(47deg) rotateZ(-5deg) skewX(-3deg); }
          73% { transform: rotateX(-16deg) rotateY(-52deg) rotateZ(8deg) skewX(2deg); }
        }
        @keyframes journeyDustFloat {
          0%,100% { transform: translate3d(0,10px,0) scale(.78); opacity: .08; }
          35% { transform: translate3d(calc(var(--dust-drift) * .35),-16px,0) scale(1); opacity: .56; }
          70% { transform: translate3d(var(--dust-drift),-38px,0) scale(.88); opacity: .2; }
        }
        ...
        @media (prefers-reduced-motion: reduce) {
          span,svg { animation-duration: 1ms !important; animation-iteration-count: 1 !important; }
        }
      `}</style>
    </section>
  );
