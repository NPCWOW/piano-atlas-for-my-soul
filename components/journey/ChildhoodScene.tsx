  ...
  return (
    ...
    >
      <div className="mx-auto flex min-h-[112vh] max-w-[1600px] items-end px-7 pb-24 pt-24 sm:px-12 lg:px-20 xl:px-24">
        <div
          className={`max-w-[640px] transition-all duration-[1500ms] ${
        ...
        >
          ...
          <p className="font-serif text-[clamp(5.6rem,12vw,10.5rem)] leading-[.78] tracking-[-.055em] text-[#e5cb8d] drop-shadow-[0_8px_30px_rgba(0,0,0,.45)]">
            1873
          </p>

          <p className="mt-8 text-[11px] uppercase tracking-[.38em] text-white/52">
            Новгородская губерния
          </p>

          <h3 className="mt-6 max-w-xl font-serif text-4xl leading-[1.02] text-[#f3ede2] sm:text-6xl lg:text-7xl">
            Детство среди русской природы
          </h3>

          <p className="mt-7 max-w-lg text-[15px] leading-7 text-white/64 sm:text-[17px] sm:leading-8">
      ...
      <div className="pointer-events-none absolute right-7 top-1/2 hidden -translate-y-1/2 flex-col items-center gap-5 text-[10px] uppercase tracking-[.2em] text-white/38 lg:flex">
        {[
          ...
          <div key={label as string} className="flex items-center gap-3">
            <span className={`h-px ${selected ? "w-7 bg-[#caa65d]" : "w-4 bg-white/16"}`} />
            <span className={selected ? "text-[#d9bd7c]" : "text-white/32"}>{label}</span>
          </div>
        ))}
      </div>
    </JourneySceneEngine>
